...
%% Scan conversion
F = griddedInterpolant({angles_rad(:), r_axis_m(:)}, polar_data, 'linear', 'none');
...
cart_data(outside) = NaN;


%% Dynamic range
cart_log10 = cart_data ./ (2 ^ params.log10_frac_bits);     % FPGA log10: Q2.14
cart_log10_max = max(cart_log10(isfinite(cart_log10)));
cart_db = 20 * (cart_log10 - cart_log10_max);
cart_db(outside | ~isfinite(cart_db)) = -params.dynamic_range_db;
cart_db = max(cart_db, -params.dynamic_range_db);
cart_db = min(cart_db, 0);

cart_uint8 = uint8(round((cart_db + params.dynamic_range_db) ./ params.dynamic_range_db .* 255));
